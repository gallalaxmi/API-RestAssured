...
public class OrderEndpointStepDefinitions {
	...
	public void i_make_a_post_request(Integer orderId) {
		...
		world.setResponse(orderEndpoint.placeOrder(world.getRequest(), new Order(orderId, 0, 1, "2021-12-16T17:15:21.774Z", "placed", false)));
	}
	...
	public void i_am_able_to_place_an_order_for_a_pet() {
		world.setResponse(orderEndpoint.placeOrder(world.getRequest(), new Order(9, world.getPet().getId(), 1, "2021-12-16T17:15:21.774Z", "placed", false)));
		orderEndpoint.verifyResponseStatusValue(world.getResponse(), orderEndpoint.SUCCESS_STATUS_CODE);
	}
	
	@Then("^I am not able to place an order for a cat$")
	public void i_am_not_able_to_place_an_order_for_a_cat() {
		world.setResponse(orderEndpoint.placeOrder(world.getRequest(), new Order(7, world.getPet().getId(), 1, "2021-12-16T17:15:21.774Z", "placed", false)));
		orderEndpoint.verifyResponseStatusValue(world.getResponse(), orderEndpoint.INVALID_STATUS_CODE);
	}
	
	@When("^I delete the order with an id value of '(.*?)'$")
	public void i_delete_the_order(String id) {
		world.setResponse(orderEndpoint.deleteOrder(world.getRequest(), id));
	}
}
